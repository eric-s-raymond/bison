...
chain_descendants_print (FILE *out)
{
  ...
  for (symbol_number sym = 0; sym < nsyms; ++sym)
    {
      fprintf (out, "descendants[%s] =", symbols[sym]->tag);
...
size_t
chain_ancestors_count (symbol_number s)
{
  size_t res = 0;
  for (symbol_number anc = 0; anc < nsyms; ++anc)
    res += bitset_test (descendants[anc], s);
  return res;
}

bool
is_leaf (symbol_number s)
{
  assert (descendants);
  return bitset_count (descendants[s]) == 1;
}
